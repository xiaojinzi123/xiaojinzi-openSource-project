...
<body>
	<div id="div_panel" class="easyui-panel" title="菜单条目列表"
		...
		<table id="t_easyui-datagrid" class="easyui-datagrid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'name',width:40,align:'center'">名称</th>
					<th data-options="field:'menu.name',width:40,align:'center'">所属菜单</th>
			...
			<tbody>
				<c:forEach items="${p.rows}" var="item">
					<tr>
						...
						<td>${item.url}</td>
						<td><a
							onclick="preShowUpdateDialog('${item.id}','${item.name}','${item.url}')"
	...
	<div id="div_entityForm" title="添加">
		<form id="entityForm">
			<table>
				<tr>
					<td>菜单:</td>
					<td><select id="menu_select" name="menu.id">
					</select></td>
				</tr>
				<tr>
					<td>名称:</td>
				...
				<tr>
					<td>请求地址:</td>
					<td><input id="entityUrl" type="text" name="url" /></td>
				</tr>
			</table>
		</form>
	</div>
...
<script type="text/javascript">
	...
	//获取所有的菜单的地址
	var getAllMenuUrl = "${basePath}/back/menu/getAllJsonMenu"
	 ...
	 * 准备弹出更新的对话框
	 */
	function preShowUpdateDialog(id, menuName, url){
		//根据地址获取所有的菜单的数据
		getJson(getAllMenuUrl, function(msg){
			//拿到菜单的集合
			var menus = msg.data;
			var htmlContent = "";
			for(var i = 0; i < menus.length; i++){
				var menu = menus[i];
				htmlContent += "<option value='"+menu.id+"'>"+menu.name+"</option>"
			}
			$('#menu_select').html(htmlContent);
			showUpdateDialog(id,menuName,url);
		})
	}
	 ...
	 * 弹出更新的对话框
	 */
	function showUpdateDialog(id, menuName,url) {
		$("#entityId").attr("value", id);
		$("#entityName").attr("value", menuName);
		$("#entityUrl").attr("value", url);
